...
def _scenario_for_setup(package: Record, setup_minutes: float) -> dict[str, Any] | None:
    ...
    return min(
        scenarios,
        key=lambda row: abs(
            float(row.get("hypothetical_total_package_setup_minutes") or 0.0)
            - setup_minutes
        ),
    )


def evaluate_copick_package_decision(
    payload: Record,
    *,
    setup_minutes: float = 15.0,
    max_payback_pickings: float = 50.0,
    lookback_days: int = 90,
    reconciliation_tolerance_ft: float = 0.001,
) -> dict[str, Any]:
    if setup_minutes <= 0:
        raise ValueError("setup_minutes must be greater than zero")
    if max_payback_pickings <= 0:
        raise ValueError("max_payback_pickings must be greater than zero")
    if lookback_days <= 0:
        raise ValueError("lookback_days must be greater than zero")
    if reconciliation_tolerance_ft < 0:
        raise ValueError("reconciliation_tolerance_ft must be zero or greater")

    reconciliation_difference = abs(
        float(
            (payload.get("summary") or {}).get(
                "shared_savings_reconciliation_difference_ft"
            )
            or 0.0
        )
    )
    ...
    for package in payload.get("packages") or []:
        scenario = _scenario_for_setup(package, setup_minutes)
        payback = (
            _number(scenario.get("payback_affected_pickings"))
        ...
        if package_saved_ft <= 0:
        ...
        elif reconciliation_difference > reconciliation_tolerance_ft:
            decision = "DEFER"
            ...
            if operational_blockers:
                decision = "DEFER"
                reasons.extend(
                    f"operational_blocker:{item}" for item in operational_blockers
                )
            elif payback is None:
                decision = "DEFER"
                reasons.append("package_payback_scenario_unavailable")
            elif payback > max_payback_pickings:
                decision = "DEFER"
        ...
        decisions.append(
            {
                ...
                "selected_decision_scenario": {
                    "requested_total_package_setup_minutes": setup_minutes,
                    ...
                    "maximum_allowed_payback_affected_pickings": max_payback_pickings,
                },
    ...
    return {
        ...
        "safe_to_execute": False,
        "decision_policy": {
            "decision_total_package_setup_minutes": setup_minutes,
            "max_payback_affected_pickings": max_payback_pickings,
            "lookback_days": lookback_days,
            "reconciliation_tolerance_ft": reconciliation_tolerance_ft,
