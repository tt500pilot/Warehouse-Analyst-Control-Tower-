...
def main() -> None:
    ...
    parser.add_argument(
        "--area-slug",
        default=None,
        help="Output filename prefix. Defaults to geometry filename without '-geometry'.",
    )
    ...
    parser.add_argument(
        ...
        default="5,15,30,60",
        help="Comma-separated hypothetical setup-time sensitivity scenarios.",
    )
    parser.add_argument(
        "--decision-setup-minutes",
        type=float,
        default=15.0,
        help="Setup-time scenario used by the final pilot decision gate.",
    )
    parser.add_argument(
        "--max-payback-pickings",
        type=float,
        default=50.0,
        help="Maximum modeled affected-picking payback allowed by the pilot decision policy.",
    )
    parser.add_argument(
        "--product-metadata",
        default=None,
        help="Optional product physical-metadata CSV for relocation readiness. If omitted, the readiness CLI default is used.",
    )
    ...
    _run(
        "mapped-area slotting analysis",
        [
            python,
            str(scripts / "analyze_mapped_aisle_slotting.py"),
            "--geometry",
            str(geometry),
            "--lookback-days",
    ...
    _run(
        "matched completed/planned route validation",
        [
            python,
            str(scripts / "validate_mapped_aisle_route_impact.py"),
            "--geometry",
            str(geometry),
            "--slotting-result",
            str(slotting),
    ...
    readiness_command = [
        python,
        str(scripts / "analyze_mapped_aisle_relocation_readiness.py"),
        "--geometry",
        str(geometry),
        "--slotting-result",
        str(slotting),
    ...
    _run(
        "relocation economics sensitivity",
        [
            python,
            str(scripts / "analyze_mapped_aisle_relocation_economics.py"),
            "--readiness",
            str(readiness),
            "--route-validation",
            str(route),
    ...
    _run(
        ...
        [
            python,
            str(scripts / "evaluate_mapped_aisle_pilot_decision.py"),
            "--readiness",
            str(readiness),
            "--economics",
            str(economics),
    ...
    _run(
        ...
        [
            python,
            str(scripts / "generate_mapped_aisle_decision_report.py"),
            "--slotting",
            str(slotting),
            "--route-validation",
            str(route),
